...
const ProductsAdd = () => {
    ...
    const cancel = () => {
        ...
        var hasChanges = productName.value.length > 0 || price.value.length > 0 || stock.value.length > 0 ||
    ...
    const save = async (event) => {
        ...
        var {productName, MSU, price, stock, MDPrice, MDPercentage} = document.forms[0];
        var errros = "";
        errros += parseInt(MDPrice.value) > (parseInt(price) / 2) ? "El descuento por precio no puede superar la mitad del unitario. \n": "";
        errros += parseInt(MDPercentage.value) > 51 ? "El descuento por descuento no puede superar el 50% \n": "";
        if(errros.length > 0){
